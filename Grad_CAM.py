...
# Created:  2017-05-26
#
# Modified to allow for A3C usage
...
class _PropagationBase(object):
    ...
    def forward(self, image, lstm):
        """Most changes are in this function to allow for lstm"""
        self.image = image
        self.model.zero_grad()

        self.critic, self.preds, _  = self.model(self.image, *lstm)

        self.probs = F.softmax(self.preds, dim=1)[0]
        self.prob, self.idx = self.probs.data.sort(0, True)
        return self.prob, self.idx

    def backward(self, idx, critic = None):
        """There is some changes here too to allow for critic backprop.
        Note that this has not been used in the thesis but it does work."""
        if critic is not None:
            c = torch.FloatTensor(1, 1).zero_()
            c[0] = critic
            self.critic.backward(gradient = c, retain_graph = True)
        else:
...
class BackPropagation(_PropagationBase):
    ...
    def generate(self):
        output = self.image.grad.data.cpu().numpy()
...
class Grad_CAM(_PropagationBase):
    ...
    def __init__(self, model):
        ...
        def func_f(module, input, output):
            if isinstance(output, tuple): output = output[1] #actor/critic
